...
#'   Every registered `packageqa.*` rule runs by default; a `config$rules`
...
#' @export
run_packageqa_scan <- function(root = ".", config = default_config()) {
  ...
  for (rule in packageqa_rules) {
    ...
    result <- tryCatch(
      rule$domain_fns$check_packageqa(root, params),
      error = function(e) {
        list(new_diagnostic(
